...
def _qcri_answer(user_prompt, history=""):
    # Removed demo-scripted QCRI/HBKU answer.
    # QCRI/HBKU should resolve through normal place_lookup / route_plan / aliases.
    return None
...
def _smalltalk_answer(user_prompt, location=None):
    ...
    if any(p in text for p in location_phrases):
        if isinstance(location, dict) and location.get("lat") and location.get("lng"):
            ...
            return (
                f"I’ve got your browser location: roughly {lat:.5f}, {lng:.5f} "
...
def frontend_index():
    ...
    # Runtime injection is intentional: it protects Ouaisse's frontend/index.html
    # from future overwrites while keeping mic/location/Aura loaded after the inline chat script.
    voice_wave = '<script src="/voice_waveform.js"></script>'
    voice_loc = '<script src="/voice_location.js"></script>'

    inject = []
    if "voice_waveform.js" not in html:
        inject.append(voice_wave)
    if "voice_location.js" not in html:
        inject.append(voice_loc)

    if inject:
        comment = (
            "<!-- Qaarib voice + Aura TTS. MUST stay loaded AFTER the inline script "
            "that defines sendMessage/appendBotText. Do not remove on frontend edits. -->"
        )
...
def aura_tts():
    ...
    try:
        r = requests.post(
            ...
            json={
                "model": AURA_TTS_MODEL,
                "input": text[:700],
                "voice": AURA_TTS_VOICE,
                "response_format": AURA_TTS_FORMAT,
                "stream": False,
            },
...
def _build_widgets(router_data, tool_results):
    ...
    for r in tool_results or []:
        ...
        if r.get("origin") and r.get("destination") and (r.get("travel_mode") or r.get("recommended_mode")):
            ...
            legs = [s.strip() for s in re.split(r"\+|→|->", mode) if s.strip()] if mode else []
            widgets.append({
                "type": "route",
                "origin": r.get("origin", ""),
...
def chat():
    ...
    try:
        ...
        direct = (
            _qcri_answer(user_prompt, history_before_turn)
            or _smalltalk_answer(user_prompt, location)
            or _extended_greeting_answer(user_prompt)
        )
        if direct:
        ...
        if pre:
            ...
            if pre.get("direct_answer"):
                ...
                if not response:
                    parts = [
                        r.get("final_answer") or r.get("summary", "")
                        for r in (tool_results or [])
                        if r.get("final_answer") or r.get("summary")
                    ]
            ...
            return jsonify({
                "response": response,
                "router": pre,
                "widgets": _build_widgets(pre, tool_results),
        ...
        if policy_answer:
            ...
            if deterministic:
                ...
                try:
                    sent_prompt = (
                        build_prompt(enriched_prompt, tool_results=tool_results, active_tool_label=active_tool_label)
                        if router_data.get("tools")
                        else _compact_fanar_prompt(enriched_prompt, history_before_turn)
                    )
        ...
        return jsonify({
            "response": response,
            "router": router_data,
            "widgets": _build_widgets(router_data, tool_results),
            "timing": {"router_ms": router_ms, "tool_ms": tool_ms, "responder_ms": responder_ms},
        })
